...
document.addEventListener("DOMContentLoaded", function (event) {
    let heros = JSON.parse(superheroJSON);
    let superherosContent = "";
    for (let hero of heros) {
        superherosContent += `
            <div class = "hero">
                <h2 class = "hero__name">${hero.name}</h2>
                <img class = "hero__btn" src="assets/images/boom.png" alt="btn">
                <div class = "hero__universe"><span class="hero__p">Вселенная: </span>${hero.universe}</div>
                <div class = "hero__alterego"><span class="hero__p">Альтер-эго: </span>${hero.alterego}</div>
                <div class = "hero__occupation"><span class="hero__p">Род деятельности: </span>${hero.occupation}</div>
                <div class = "hero__friends"><span class="hero__p">Друзья: </span>${hero.friends}</div>
                <div class = "hero__superpowers"><span class="hero__p">Суперсилы: </span>${hero.superpowers}</div>
                <img class = "hero__img" src = ${hero.url}>
                <div class = "hero__info"><span class="hero__p">Информация: </span>${hero.info}</div>

                <form action="" class="hero__form_stars">
                    <label class="container__label" for=""><input type="radio" value="1" name="raiting" class="container__input">
                        <svg class="icon">
                            <use xlink:href="assets/icons/sprite.svg#star-full-icon"></use>
                        </svg>
                    </label>
                    <label class="container__label" for=""><input type="radio" value="2" name="raiting" class="container__input">
                        <svg class="icon">
                            <use xlink:href="assets/icons/sprite.svg#star-full-icon"></use>
                        </svg>
                    </label>
                    <label class="container__label" for=""><input type="radio" value="3" name="raiting" class="container__input">
                        <svg class="icon">
                            <use xlink:href="assets/icons/sprite.svg#star-full-icon"></use>
                        </svg>
                    </label>
                    <label class="container__label" for=""><input type="radio" value="4" name="raiting" class="container__input">
                        <svg class="icon">
                            <use xlink:href="assets/icons/sprite.svg#star-full-icon"></use>
                        </svg>
                    </label>
                    <label class="container__label" for=""><input type="radio" value="5" name="raiting" class="container__input">
                        <svg class="icon">
                            <use xlink:href="assets/icons/sprite.svg#star-full-icon"></use>
                        </svg>
                    </label>
                    <label class="container__label" for=""><input type="radio" value="6" name="raiting" class="container__input">
                    <svg class="icon">
                        <use xlink:href="assets/icons/sprite.svg#star-full-icon"></use>
                    </svg>
                    </label>
                    <label class="container__label" for=""><input type="radio" value="7" name="raiting" class="container__input">
                        <svg class="icon">
                            <use xlink:href="assets/icons/sprite.svg#star-full-icon"></use>
                        </svg>
                    </label>
                    <label class="container__label" for=""><input type="radio" value="8" name="raiting" class="container__input">
                        <svg class="icon">
                            <use xlink:href="assets/icons/sprite.svg#star-full-icon"></use>
                        </svg>
                    </label>
                    <label class="container__label" for=""><input type="radio" value="9" name="raiting" class="container__input">
                        <svg class="icon">
                            <use xlink:href="assets/icons/sprite.svg#star-full-icon"></use>
                        </svg>
                    </label>
                    <label class="container__label" for=""><input type="radio" value="10" name="raiting" class="container__input">
                        <svg class="icon">
                            <use xlink:href="assets/icons/sprite.svg#star-full-icon"></use>
                        </svg>
                    </label>
    ...
    document.getElementById("superheroContainer").innerHTML = superherosContent;

    const heroContainer = document.querySelector('#superheroContainer');

    heroContainer.addEventListener('click', (event) => {
        // клик для смены информации
        if (event.target.classList.contains('hero__btn')) {
            const parentDiv = event.target.parentNode; //ищем родителя элемента по которому произошел клик
            let hero__img = parentDiv.querySelector(".hero__img");
            let hero__info = parentDiv.querySelector(".hero__info");
            let hero__universe = parentDiv.querySelector(".hero__universe");
            let hero__alterego = parentDiv.querySelector(".hero__alterego");
            let hero__occupation = parentDiv.querySelector(".hero__occupation");
            let hero__friends = parentDiv.querySelector(".hero__friends");
            let hero__superpowers = parentDiv.querySelector(".hero__superpowers");

            function changeInfo() {
                let isVisible = hero__info.style.display == "block";
                let isHero = hero__img.style.display == "none" || hero__universe.style.display == "none" || hero__alterego.style.display == "none" || hero__occupation.style.display == "none" || hero__friends.style.display == "none" || hero__superpowers.style.display == "none";

                hero__info.style.display = isVisible ? "none" : "block";
                hero__img.style.display = isHero ? "block" : "none";
                hero__universe.style.display = isHero ? "block" : "none";
                hero__alterego.style.display = isHero ? "block" : "none";
                hero__occupation.style.display = isHero ? "block" : "none";
                hero__friends.style.display = isHero ? "block" : "none";
                hero__superpowers.style.display = isHero ? "block" : "none";
            }
            changeInfo();
        }
        ...
        if (event.target.classList.contains('container__input')) {
            let parentStars = event.target.parentNode.parentNode.parentNode;
            let hero__name = parentStars.querySelector(".hero__name").textContent;
            //console.log(hero__name);
            let inputs = parentStars.querySelectorAll('.container__input');
            let icons = parentStars.querySelectorAll('.icon');
